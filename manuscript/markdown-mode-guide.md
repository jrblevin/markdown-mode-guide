...
![Live Preview Mode with `eww`](images/live-preview.png)

Markdown Mode also includes a live preview mode, which opens a preview
window in `eww`, which is a built-in browser available in Emacs.
A timer periodically updates the HTML generated by `markdown-command`.
Press `C-c C-c l` to toggle `markdown-live-preview-mode`.
